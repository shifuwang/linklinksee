...
import os  # 打开文件会用到的包
import random  #随机数
import tkinter as tk   #图形界面包
import tkinter.messagebox   # 对话框
from PIL import Image, ImageTk  #  图片处理包

class MainWindow():
	__gameTitle = "LinklinkSee"
	__windowWidth = 700    #窗口的高度 和 宽度
	__windowHeigth = 500
	__icons = []
	__gameSize = 10 # 游戏尺寸 长宽方块个数
	__iconKind = __gameSize * __gameSize / 4 # 小图片种类数量   相当于
	__iconWidth = 40
	__iconHeight = 40
	__map = [] # 游戏地图
	__delta = 25
	__isFirst = True     # 一些标记变量
	__isGameStart = False
	__formerPoint = None
	EMPTY = -1
	NONE_LINK = 0
	...
	# 两方块相连的路径
	STRAIGHT_LINK = 1	       #直线路径
	ONE_CORNER_LINK = 2	# 1折
	TWO_CORNER_LINK = 3	# 2折

	def __init__(self):
		self.root = tk.Tk()
		self.root.title(self.__gameTitle)
		self.centerWindow(self.__windowWidth, self.__windowHeigth)
		self.root.minsize(460, 460)	# 设置窗口最小尺寸
